...
log "Building container (dev-speech2text)..."
(
  cd "$ROOT_DIR"
  if [[ "$NO_CACHE" == "1" ]]; then
    docker compose build --no-cache dev-speech2text
  else
    docker compose build dev-speech2text
  fi
)

log "Starting container (dev-speech2text)..."
(
  cd "$ROOT_DIR"
...
if [[ -f "$MISSING_REQ" ]]; then
  ...
  (
    cd "$ROOT_DIR"
    docker compose run --rm dev-speech2text bash -lc "pip install -r /workspace/scripts/missing-deps.txt"
  )
fi
